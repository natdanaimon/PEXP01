...
//        $CSS .= "                background-size: 75px 75px; ";
    ...
    function resp($statusCode, $statusDesc = NULL, $object = NULL) {
        $respErrCode = "";
//        if ($statusCode != 0) {
//            $respErrCode = "  ( CODE : " . $statusCode . ")";
//        }
        
        if ($object != NULL) {
            $tmp = array(
                'statusCode' => $statusCode,
                'statusDesc' => $statusDesc . $respErrCode,
                'bean' => $object,
            );
        } else {
            $tmp = array(
    ...
    function setMsg($field, $err, $labelFormat = NULL) {
        ...
        switch ($err) {
            case L::Err_2002:
                $response = eregi_replace("field", $field, $err);
                break;
            case L::Err_2003:
                $response = eregi_replace("field", $field, $err);
                $response = eregi_replace("regular", $labelFormat, $response);
                break;
            case L::Err_2004:
                $response = eregi_replace("field", $field, $err);
                break;
